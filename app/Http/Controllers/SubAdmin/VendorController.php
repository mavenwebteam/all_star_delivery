...
class VendorController extends Controller {
  ...
  public function __construct()
	...
	{	
		...
		if ($request->ajax()) 
		{
			return view('sub_admin.vendor.search', compact('userdata'));  
        }
		$admindata = Admin::find(session('admin')['id']);
        return view('sub_admin.vendor.show', compact('userdata','admindata'));
	}
	...
	{   
		...
		return view('sub_admin.vendor.add',compact('countrycode_box'));
	} 
...

	{
		...
		return view('sub_admin.vendor.edit',["userdata" => $userdata,'countrycode_box'=> $countrycode_box]);
	...
	public function viewuser($id)
	{
		$userId = base64_decode($id);
		$userdata = User::find($userId);
		return view('sub_admin.vendor.view',["userdata" => $userdata]);
