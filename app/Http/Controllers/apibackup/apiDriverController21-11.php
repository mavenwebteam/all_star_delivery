					...
					if($userArray){
						if ($userArray->id!="")
						{
							...
							else
							{
								...
								if($userArray->type == 2){
									$deliveryboyorder = DB::table('orders')->where('delivery_boy_id',$userArray->id)->where('delivery_boy_status',3)->count();
									
									$runingorder = DB::table('orders')->where('delivery_boy_id',$userArray->id)->where('delivery_boy_status',1)->count();
									
								}elseif($userArray->type == 3){
									$deliveryboyorder = DB::table('orders')->where('picker_id',$userArray->id)->where('picker_status',3)->count();
									
									$runingorder = DB::table('orders')->where('picker_id',$userArray->id)->where('picker_status',1)->count();		
								...
								$data['rating'] = $userArray->rating;
								$data['point'] = $userArray->point;
								$data['wallet_amount'] = $userArray->wallet_amount;
								$data['total_order'] = !empty($deliveryboyorder) ? $deliveryboyorder : 0;
								$data['runing_order'] = !empty($runingorder) ? $runingorder : 0;
...
public function jobList(Request $request){ 
	...
	if($auth_token)
	{
					...
					if($userArray){
								...
								$orderdata = Orders::select('orders.id','orders.picker_id','orders.picker_status','orders.delivery_boy_id','orders.delivery_boy_status','orders.status','orders.order_address','orders.order_completed_date',
								'orders.net_amount','orders.product_id','orders.payment_mode','orders.total_amount','orders.vat_tax',
								'orders.total_shipping_amount','orders.instructions','orders.order_delivery_status','orders.is_cancelled','orders.created_at','stores.name',
							...
							//echo '<pre>'; print_r($orderdata); die;
								...
								{    
								   $users = array("full_name" => !empty($data->full_name) ? $data->full_name : '',					
									...
									$orderitems = Orderitems::select('order_items.*','products.name','products.price as product_price',
									'products.discount_price','category.name as cat_name','products.weight','products.weight_unit')
									->where('order_items.order_id',$data->id)
									->leftJoin('products', 'order_items.product_id', '=', 'products.id')
								...
								$data1['list'][]=array('store_name'=>!empty($data->name) ? $data->name : '','image'=>URL::to('/media/store').'/'.$data->image,
									...
									'payment_mode'=>!empty($payment_type) ? $payment_type : '',
									'total_amount'=>!empty($data->total_amount) ? $data->total_amount : '0.00',
									'vat_tax'=>!empty($data->vat_tax) ? $data->vat_tax : '0.00',
									'total_shipping_amount'=>!empty($data->total_shipping_amount) ? $data->total_shipping_amount : '0.00',
									'net_amount'=>!empty($data->net_amount) ? $data->net_amount : '0.00',
									'rating'=>$rating,
									'order_delivery_status' =>  !empty($data->order_delivery_status) ? $data->order_delivery_status : 0,
