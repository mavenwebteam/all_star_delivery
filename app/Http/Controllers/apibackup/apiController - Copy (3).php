	...
	public function home(Request $request) 
	{
		...
		if ($decoded)
		{
			if(!empty($decoded['lat']) && !empty($decoded['long'])) 
			{
				...
				/*$distance_query=$gr_circle_radius.' * acos( cos( radians('.$lat.') ) * cos( radians( lat ) ) * cos( radians( lng ) - radians('.$long.') ) + sin( radians('.$lat.') ) * sin( radians( lat ) ) ) )';
...
$store_data = Stores::select('stores.id','stores.offer_code','stores.offer_per',
				...
				if(!empty($user_id) && !empty($previous_orders))
					{
						$store_data = $store_data->where('orders.orderuser_id', $user_id);
					}			
				...
				if($store_data->count()>0)
				{
					...
					foreach($store_data as $data)
					{
						...
						,'rating'=>(string)$data->rating,'is_open'=>$data->is_open,'offer_code'=>$data->offer_code,'is_faviourte' => !empty($StoreFaviourite) ? 1 : 0, 'offer'=>$data->offer_per.'%','address'=> !empty($data->address) ? $data->address :'','product_count'=>$product_category_count,'product_category'=>$product_category_data);
   ...
   public function store_detail(Request $request) 
	{
		...
		if ($decoded)
		{
			if(!empty($decoded['store_id']) && !empty($decoded['category_id'])) 
			{
				if(isset($decoded['user_id']) && $decoded['user_id']!=""){
					$usercartitemcount = Cartitem::where('user_id',$decoded['user_id'])->count();
					$store_detail_data['count'] = (string)$usercartitemcount;
					$usercartitems = Cartitem::select('cart_item.store_id','cart_item.id','cart_item.product_id','cart_item.user_id',
				...
				$store_data = Stores::select('id')->where('id',$store_id)->where('stores.status','1')->first();
				
			//	echo '<pre>';print_r($store_data);exit;
			    if(!empty($decoded['user_id']))
				{   
					$UserFaviourite = DB::table('favorites')->where('user_id',$decoded['user_id'])->where('store_id',$decoded['store_id'])->pluck('store_id')->all();
				...
				{  
					...
					,'product_inventories.discount_price','product_images.image')
					...
					->where('products.store_id',$store_id)->whereNotNull('product_inventories.product_id')->where('products.status','1')->orderBy("products.id","ASC");
					...
					if($product_data->count()>0)
					{
						foreach($product_data as $data)
						{
							...
							$store_product_data[] =  array('id' =>$data->id,'product_name' =>$data->product_name,'category_id' =>$data->cat_id,'category_name' =>$data->category_name,'image'=>URL::to('/media/products').'/'.$data->image,'store_name'=>$data->store_name
							,'orignal_price' =>$data->price,'discount_price' =>$data->discount_price,'offer'=>round((($data->price - $data->discount_price)*100) /$data->price).'%','cart_is'=>$cart_is,'cart_qty'=>$cart_qty,'is_faviourte' => !empty($ProductFaviourite) ? 1 : 0);
...
public function verify_otp() {
	...
	public function userlocationslist() {
		header('Content-Type: application/json');
		$status = 0;
		$message = NULL;
		$data = array();
		...
		//$decoded        = $_REQUEST;
				...
				if($decoded) {
					if(!empty($decoded['user_id']) ) 
					{
						...
						$userArray = Userlocations::where('user_id','=',$decoded['user_id'])->get();
						//dd($userArray);
					if(!empty($userArray)){
		...
		if ($decoded)
		{
			if(!empty($decoded['lat']) && !empty($decoded['long']) && !empty($decoded['user_id'])) 
			{
				$lat = $decoded['lat'];
				$long = $decoded['long'];
				//$store_data = Stores::orderBy("id","ASC")->where('stores.status','1')->paginate(10);
				//$store_count_data = Stores::orderBy("id","ASC")->where('stores.status','1')->count();
...
public function favourite(){
	...
	public function AddToCart() {
		...
		//$decoded        = $_REQUEST;
				...
				if($decoded) {
					if(!empty($decoded['user_id']) && !empty($decoded['product_id']) && !empty($decoded['qty']) && !empty($decoded['price']) &&  !empty($decoded['store_id'])) 
					{
						$r=0;
						if($r==0){
							$checkcartitem = Cartitem::where('user_id',$decoded['user_id'])->where('product_id',$decoded['product_id'])->where('store_id',$decoded['store_id'])->first();
							//dd($checkcartitem);
							//echo 1; exit;
							...
							}else
							{
								...
								$itemdel = Cartitem::whereUser_id($decoded['user_id'])->where('store_id','!=',$decoded['store_id']);
								if(	$itemdel )
								{
									$itemdel->delete();
								}
	...
	public function CheckOut() {
		...
		//$decoded        = $_REQUEST;
				...
				if($decoded) {
					if(!empty($decoded['delivery_type']) && !empty($decoded['payment_mode']) && !empty($decoded['user_id']) && !empty($decoded['store_id'])  && !empty($decoded['address']) ) 
					{
						$r=0;
	...
	public function StoreOffers() {
		...
		//$decoded        = $_REQUEST;
						...
						$StoreOffers = Storeoffers::select('store_offers.id','store_offers.store_id' ,'store_offers.offer_percantage' 
						,'store_offers.expiry_date' ,'store_offers.offer_description','store_offers.offer_code','stores.name','stores.image')
						...
						->paginate(10);
							...
							foreach( $StoreOffers as $data1){
								if($data1->image!=""){
									$store_image=URL::to('/media/store').'/'.$data1->image;
								}else{
									$store_image="";
								}
								$expiry_date=date('d M Y',strtotime($data1->expiry_date));
								$data[] = array('id' => $data1->id,
								'store_id' => $data1->store_id,
								'offer_code' => $data1->offer_code,
								'offer_percantage' =>$data1->offer_percantage.'%',
								'expiry_date'=>$expiry_date,
								'store_name'=>$data1->name,
								'store_image'=>$store_image,
								'offer_description'=>$data1->offer_description,
