...
class OrderController extends BaseController
{
        ...
        try {
            ...
            if($request->address_id){
                ...
                if($address){
                    ...
                    return $this->sendError(trans('Delivery not possible on this address!'),JsonResponse::HTTP_UNPROCESSABLE_ENTITY );
                }
            }else{
    ...
    public function getDeliveryCharges($distance, $feeFor, $tax=NULL)
    {
        ...
        return round(number_format($deliveryCharge, 2, '.', ''));
    }
    ...
    */
    public function placeOrder(PlaceOrderRequest $request)
    {
        ...
        $is_open = Stores::where('id',$request->store_id)->where('is_open','open')
                                         ->whereTime('open_at','<=' ,$current_time)
                                         ->whereTime('close_at','>=' ,$current_time)->count();
        
        if($is_open !== 1){
            return $this->sendError(trans('message.STORE_CLOSE'), JsonResponse::HTTP_UNPROCESSABLE_ENTITY );
            die;
        }
        ...
        if($minOrderAmount['is_check']){
            $msg = trans('message.MIN_ORDER_AMOUNT', ['AMOUNT' => $minOrderAmount['amount']]);
        ...
        $data = [
            ...
            'delivery_fee' => round($request->delivery_fee),
            'tax' => $request->tax,
        ...
        $messageTitle = 'Net order: '.$order->order_id;
        $messageBody = 'Got new OrderId: '.$order->order_id .", need to accept the order!";
        $url = "";
        $orderId = $order->order_id;
        $reciverId = $store->user_id; //vendor id
        parent::sendWebNotification($reciverId ,$orderId, $messageTitle, $messageBody, $url);        
	...
	{		
		$distance_query = '(6371* acos( cos( radians('.$latitude.') ) * cos( radians( users.latitude ) )
        ...
        if($radius <= $adminSetting->driver_range_3)
        {
            ...
            if($order && empty($order->driver_id) && $order->status !== 'CANCELLED')
            {
                ...
                if(empty($latitude) ||empty($longitude)){
                    ...
                    WebPushNotification::dispatch($token, $title, $body, 'order', $orderId);
                    die();
                }
             ...
             if(empty($order->driver_id)){
                 $messageTitle = 'Driver not found for OrderId:'.$order->id;
                 $messageBody = 'Driver not found for OrderId:'.$order->id .", need to assign driver manualy";
                 $url = "";
                 $reciverId = 1; // admin Id
                 parent::sendWebNotification($reciverId, $orderId, $messageTitle, $messageBody, $url);
             }
             return true;
        }
    ...
    public function orderHistory(Request $request)
    {
        try {
            ...
            if($request->order_type == 'CURRENT_ORDER'){
                $orders = $order->whereNotIn('status',['DELIVERED','CANCELLED'])->latest('updated_at')->paginate(Constant::API_RECORD_PER_PAGE);
            }else{
                $orders = $order->whereIn('status',['DELIVERED','CANCELLED'])->latest('updated_at')->paginate(Constant::API_RECORD_PER_PAGE);
            }
    ...
    public function orderDetails(Request $request)
    {
        try {
            ...
            $productSum = OrderItem::where('order_id', $request->order_id)->sum('price');
            $order = Order::with(['orderItems.product','address'])->where('id', $request->order_id)
            ->first();
            $order->order_item_sum = $productSum;
    ...
    public function reOrder(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                'order_id' => 'required|exists:orders,id',
                'cart_flag' => 'required|in:0,1',  // 1=> delete existing cart and create new
            ]);
            if ($validator->fails())
            {
                return $this->sendError(trans($validator->errors()->first()),JsonResponse::HTTP_UNPROCESSABLE_ENTITY);
            ...
            $userId = Auth::id();
            if($request->cart_flag == 1){
                Cart::where('user_id', $userId)->delete();
            }else{
                $cart = Cart::where('user_id', $userId)->count();
                if($cart > 0){
                    return $this->sendError(trans('message.CART_CONTAN_ITEM'),JsonResponse::HTTP_UNPROCESSABLE_ENTITY);
                }
            }
            $items = OrderItem::with('product')->where('order_id', $request->order_id)->get();
            foreach ($items as $item){
                $latestPrice = (object_get($item, 'product.discounted_price', NULL)) ? $item->product->discounted_price : $item->product->price;
                if($latestPrice){
                    Cart::Create([
                        'user_id'    => $userId,
                        ...
                        'qty'      => $item->qty,
                        'price'    => $latestPrice,
                        'store_id' => object_get($item, 'product.store_id', NULL),
                        'amount'   => $latestPrice*$item->qty,
                        ]);
                }
            }
