...
<table class="table table-bordered table-hover">
    <tbody>
		...
		<tr> <th>Shipping Charge ($)</th><td>{{$orderdata->total_shipping_amount + $orderdata->picker_amount}}</td></tr>
        ...
        <tr> <th>Vendor Name</th><td>{{$orderdata->vendorname}}</td></tr>
		...
		<tr> <th>Order Delivery Status</th><td>
        @if($data->view_order_status=="1")
     @if($data->is_cancelled == 0)
	   @if($data->order_delivery_status == 0) Order Request Generated
 @elseif($data->order_delivery_status == 1) Accepted by store 
 @elseif($data->order_delivery_status == 2) Delivery Person Arrived
  @elseif($data->order_delivery_status == 3) Picked up and flying to you 
  @elseif($data->order_delivery_status == 4) Arrived @else Completed @endif
       @else
		    Cancel order
    @endif 
  @endif</td></tr>
