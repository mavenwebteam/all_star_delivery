    ...
    <thead>
      ...
      <tr>
        ...
        <td>{{round($store_rating,1)}}</td>
		 <td width="20%">{{$data->outlet_review}}</td>
      ...
      @if($i<2)
      <tr>
      <td colspan="5">No Vendor Review Data</td>
      </tr>
      @endif    
      </tbody> 
    </table>{!! $review_data->links() !!} Records {{ $review_data->firstItem() }} - {{ $review_data->lastItem() }} of {{ $review_data->total() }} (for page {{ $review_data->currentPage() }} )