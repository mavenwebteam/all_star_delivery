...
<script type="text/javascript">
    ...
    function search() {
        ...
        $.ajax({
            type: 'POST',
            url: '{{ URL::to('/vendor/outlet-manager') }}',
    ...
    function exportData() {
        ...
        window.location.href = '{{ URL::to('/vendor/export-outlet') }}'+'?user_search_name='+user_search_name+
    ...
    function add_record() {
		...
		var checklogin1 = checklogin();
		if(checklogin1  == true){
        $('#UserModal').html(''); $(".form-title").text('Add Outlet');
        $('#UserModal').load('{{ URL::to('/vendor/add-outlet') }}');
        $("#myModal").modal();
		}else{
			location.reload();
				$.LoadingOverlay("hide");
    ...
    function edit_record(edit_id) {
		...
		var checklogin1 = checklogin();
		if(checklogin1  == true){
        $('#UserModal').html(''); $(".form-title").text('Edit Outlet');
        $('#UserModal').load('{{ URL::to('/vendor/edit-outlet') }}'+'/'+edit_id);
        $("#myModal").modal();
		}else{
			location.reload();
				$.LoadingOverlay("hide");
    ...
    function view_record(view_id) {
		...
		var checklogin1 = checklogin();
		if(checklogin1  == true){
        $('#UserModal').html(''); $(".form-title").text('View Outlet');
        $('#UserModal').load('{{ URL::to('/vendor/view-outlet') }}'+'/'+view_id);
        $("#myModal").modal();
		}else{
			location.reload();
				$.LoadingOverlay("hide");
    ...
    function statusChange(id) 
    {
      ...
      $.ajax({
            ...
            data: { id:id}, 
            type: "POST",
            url: '{{ URL::to('/vendor/outlet-status') }}',
        }).done(function( data ) 
        {   
          search();
    ...
    $(document).ready(function() {
        loadPiece( '{{ URL::to('/vendor/outlet-manager') }}');
    })
function remove_record(id) 
	...
	var checklogin1 = checklogin();
      ...
      $.ajax({
            ...
            data: { id:id}, 
            type: "POST",
            url: '{{ URL::to('/vendor/outlet/outlet-remove') }}',
        }).done(function( data ) 
        {   
          search();
           if(data.class == 'success')
    ...
    <section class="" style="padding: 15px;">
      <div class="row">
        <div class="col-xs-12 page-user">
          <div class="box">
            <div class="box-body">
            {!! Form::open(array('url' => '/vendor/outlet-manager', 'method' => 'post','name'=>'mySearchForm','files'=>true,'novalidate' => 'novalidate','id' => 'mySearchForm')) !!}
            ...
            {!! Form::text('first_name',null, ['class' => 'form-control','id'=>'','placeholder'=>'First Name']) !!}
		               ...
		               <div class="col-xs-12 col-sm-6 col-md-3">
            <label></label>
            {!! Form::text('last_name',null, ['class' => 'form-control','id'=>'','placeholder'=>'Last Name']) !!}
           </div>
           ...
           <div class="col-xs-12 col-sm-6 col-md-12">
                ...
                <a href="javascript:" style="margin-top: 20px;" class="custom-btn pull-right btn btn-outline-primary btn-labeled" onclick="add_record();" title="Add Outlet"><span class="btn-label icon fa fa-plus"></span>Add Outlet</a>
          </div>
            {!! Form::close() !!}
            </div>
        </div>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Outlet Data Table</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body" id="replace-div">
                 @include('vendor.outlet.search')
            </div>
            <!-- /.box-body -->
          </div>
