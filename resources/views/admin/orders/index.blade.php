  ...
  <script type="text/javascript">
    ...
    */
    function orderStatusUpdate(id, status) 
    {
      if(confirm(`Are you sure want to ${status}`))
      {
        $.ajax({
          ...
          data: { id:id, status}, 
          type: "PUT",
          url: '{{ URL::to('/admin/orders/update/status') }}'+'/'+id,
    ...
    $(document).on('click','.viewOrderBtn', function(){
      ...
      $('#modelBody').load('{{ URL::to('admin/orders') }}'+'/'+orderId);
      $("#modal-lg").modal(); 
      $.LoadingOverlay("hide");
    });
    ...
    $(document).on('click','.editOrderBtn', function(){
      ...
      const orderId = modelAction(thisObj, modalTitle);
      $('#modelBody').load('{{ URL::to('admin/orders') }}'+'/'+orderId+'/edit');
      $("#modal-lg").modal(); 
      $.LoadingOverlay("hide");
    });
    ...
    $('#storeListSelect2').select2({
      ...
      ajax: {
            url: '{{ route("admin.select2.stores") }}',
            dataType: 'json',
            delay: 250,
            pagination: {
                  more: true
