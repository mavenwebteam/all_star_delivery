...
@endif  
  ...
  @if($i<2)
  <tr>
<td>No Notification Data</td>
  </tr>
  @endif    
  </tbody> 
    </table>{!! $noti_data->links() !!} <style>	span.pagecounts {    display: inline-block;    width: 100%;			}	</style>	<span class="pagecounts">Records {{ $noti_data->firstItem() }} - {{ $noti_data->lastItem() }} of {{ $noti_data->total() }} (for page {{ $noti_data->currentPage() }} ) </span>