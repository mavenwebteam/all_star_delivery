      ...
      @foreach($catedata as $data)
        <tr>
            ...
            <td class="res-dropdown" style="" align="center">
              @if(in_array('subAdmin.edit-item-category.edit', $permissionData))
              <a data-toggle="tooltip" data-placement="top" title="" href="javascript:" class="btn btn-primary" data-original-title="Edit Category" onclick="edit_record('{{base64_encode($data->id)}}')" ><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
              @endif
              @if(in_array('subAdmin.view-item-category.show', $permissionData))
              ...
              @endif
              @if(in_array('subAdmin.item-category.status', $permissionData))
