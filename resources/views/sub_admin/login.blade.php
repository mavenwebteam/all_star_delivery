...
<head>
<meta charset="utf-8">
<title>{{Config::get("Site.title")}} - SubAdmin Login</title>
    ...
    <div class="row">
      <div class="col-sm-8 col-md-6 col-sm-offset-2  col-md-offset-3" ng-app="mainApp" ng-controller="loginController">
        ...
        <div class="contsct-form">
          {!! Form::open(array('url' => 'sub-admin/login-post', 'method' => 'post','name'=>'loginForm','novalidate' => 'novalidate')) !!}          
            <span class="login_field">
              <img  src="{{asset('/admin_assets/images/user-fild.png') }}" />
              <input type="email" value='{{isset( $_COOKIE["email"]) ?  $_COOKIE["email"] : ''}}'  name="email" class="user-fild" id="email" placeholder="Email"/>
          ...
          {!! Form::close() !!}
          <p class="forgot_pass"><a href="{{ URL::to('/sub-admin/forgot-password') }}">Forgot Password ?</a></p>  
        </div>
      </div>
    </div>
  </div>
